...
def  main():
  ...
  installer = "\installer\NAMpanion Installer.exe"
   
  if demo:
    installer = "\installer\NAMpanion Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\NAMpanion manual.pdf" 
  ]
  ...
  ZIPNAME = "NAMpanion-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "NAMpanion-v" + FULLVERSIONSTR + "-win-demo.zip"
